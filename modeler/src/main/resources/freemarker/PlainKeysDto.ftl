...
public class ${className} {
  <#list model.columns as column>
   ...
   public ${className}(<#if model.primaryKey??><#list model.primaryKey.columns as column>final ${columnTypeMapping["${column.properties.DATA_TYPE}"]} ${column.properties.JAVA_NAME}<#if column_has_next>,</#if></#list></#if>) {
      <#if model.primaryKey??>
...
<#list model.columns as column>
  ...
  public void set${column.properties.JAVA_NAME?cap_first}(final ${columnTypeMapping["${column.properties.DATA_TYPE}"]} ${column.properties.JAVA_NAME}) {
    this.${column.properties.JAVA_NAME} = ${column.properties.JAVA_NAME};
  }
